...
{
    public class DeliveredOrderSubscriber : ISubscriber
    {
        public void Start(IApplicationBuilder app, IMessagingSettings messagingSettings)
        {
            Task.Factory.StartNew(() =>
            {
                new MessagingService(messagingSettings).Subscribe<Order>("ProductAPI.DeliveredOrderSubscriber", "deliveredOrder", (order) =>
                {
